...
### 권한
- [ ] **설정 토글이 권한 상태를 안 따른다** — `isEnabled`가 UserDefaults 값이라,
      기기 설정에서 알림을 끈 뒤 앱을 다시 열면 토글이 켜진 채로 보인다.
      실제 버그다
- [ ] 앱 시작 시 필요한 권한을 모두 요청 — `RootTabView.swift:306`에
      `// await notificationScheduler.requestAuthorizationOnFirstLaunch() // TEMP`로
      주석 처리돼 있다. 위 "앱 시작 시 알림 권한 요청"(2026-08-11)과 같은 건이다
- [ ] 튜토리얼과 겹치는 순서 문제는 코드 주석에 이미 설계가 적혀 있다 —
      되살릴 때 그 설계를 따를지 정해야 한다
