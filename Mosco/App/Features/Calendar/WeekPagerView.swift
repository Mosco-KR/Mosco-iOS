...
/// 압축 상태(날짜를 고른 상태)의 주간 스트립을 좌우로 넘기는 페이저.
///
/// `MonthPagerView`와 같은 구조를 주 단위로 적용한 것이다 — 창을 고정 배열로 두고
/// `.scrollTargetBehavior(.paging)`에 맡긴다. 예전에는 `scrollDisabled(true)` 상태에서
/// "40pt 넘게 밀면 ±7일 점프"였기 때문에 손가락을 따라오지 않았고, 게다가 주가
/// 바뀌는 모습이 세로 애니메이션으로 보였다.
