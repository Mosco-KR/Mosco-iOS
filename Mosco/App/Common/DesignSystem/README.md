...
- **고른 색을 거부하지 않는다.** 흰색을 골라도 막지 않고, 흰 배경에서 읽을 수
  ...
  색을 잘못 고른 줄 안다.
- **고른 즉시 바뀌지 않는다.** 미리보기로 두고 '적용'에서 바꾼다. 고르는 동안
  앱 전체가 따라 바뀌면 무엇을 고르는 중인지가 흐려지고, 스치듯 지난 색이 그대로
  적용된 것처럼 보인다. 미리보기는 **실제로 칠해질 색**(눌린 뒤의 색)으로 보여준다 —
  고른 색 그대로 보여주면 적용 뒤에 다른 색이 나온다.
