...
### R1 · 빌드·테스트가 통과하고, 바꿨다고 말한 것이 실제로 들어갔는지 확인한 뒤 보고한다
담당 지표 M3 빌드 실패율, M1 재지시율
...
(2026-08-19 추가). v1.2.0에서 두 번 걸렸다 — 편집 스크립트가 앞쪽 파일에서 실패해
뒤쪽 수정이 통째로 안 돌았는데 빌드는 통과했고, 위젯이 읽는 코드가 한 줄도 없는데
"위젯도 같이 바뀌어요"를 화면 문구로 적었다. 둘 다 사람이 잡았다.

그래서 보고 전에 둘을 확인한다.

- **스크립트로 편집했으면 결과를 되읽는다.** `grep`으로 바뀐 자리를 확인한다.
  편집 도구는 실패하면 에러를 내지만, 스크립트는 조용히 일부만 하고 끝날 수 있다.
- **동작을 문구로 약속했으면 그 동작의 코드 경로를 확인한다.** "~도 같이 바뀌어요"를
  적기 전에 그것을 읽는 코드가 실제로 있는지 본다.
...
**검토는 매번, 남기는 것은 판단이다** (2026-08-22 조정). 예전엔 "같은 종류가 반복될
때만"이었는데, 판단이 필요한 규칙은 판단하기 전에 잊힌다 — v1.3.0에서 **다섯 번
되묻고 한 번도 안 남겼다.** 그래서 되물어 답을 받은 직후에 "남길 것이 있나"를
**매번 본다.** 없으면 안 남긴다. 두 번째로 같은 종류가 갈리면 그때는 반드시 남긴다.
...
**규범이 걸린 영역(색·문구·컴포넌트)을 만졌으면 무엇을 읽고 어떻게 지켰는지 한 줄
적는다** (2026-08-19 추가). v1.2.0에서 두 번 어기고도 말하지 않았다 — 시간표 블록에
색을 두 군데 쓰고("색은 한 행에 한 번" 위반), 설정 부연을 두 문장으로 썼다(문구 원칙 3).
둘 다 사람 눈에 먼저 걸렸다.

**M8이 0이어도 규범을 어길 수 있다.** M8은 규범 문서가 없는 심볼을 가리키는지만 보고
**코드가 규범을 어기는지는 안 본다.** 지표가 덮는 범위를 규칙이 덮는 범위로 착각하지 않는다.
